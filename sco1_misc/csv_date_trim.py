...
def _trim_csv(log_filepath: Path, out_filepath: Path, test_date: dt.date) -> None:
...
@click.option(
    "--out_filename", type=str, help="Output filename. Defaults to <in_filename>_<test_date>.csv"
)
def trim_cli(
    ...
    trimmed data matching `--out_filename` will be discarded. Attempting to overwrite the input CSV
    ...
    _trim_csv(log_filepath, out_filepath, test_date)
